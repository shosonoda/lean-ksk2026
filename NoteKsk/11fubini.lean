...
theorem productMeasure_apply_symm [SFinite μ] [SFinite ν]
    {E : Set (α × β)} (hE : MeasurableSet E) :
    (μ.prod ν) E = ∫⁻ y, μ (ySection E y) ∂ν := by
  simpa [ySection] using MeasureTheory.Measure.prod_apply_symm (μ := μ) (ν := ν) hE
...
theorem measurable_measure_ySection [SFinite μ] {E : Set (α × β)} (hE : MeasurableSet E) :
    Measurable fun y : β => μ (ySection E y) := by
  simpa [ySection] using measurable_measure_prodMk_right (μ := μ) hE
